# ── compiler flags ──────────────────────────────────────
CC      ?= gcc
CFLAGS  := -std=c11 -Wall -Wextra -O2 -Iinclude -Ithird_party -Ithird_party -Ithird_party/civetweb -DNO_SSL -DUSE_IPV6 -DUSE_WEBSOCKET
LDFLAGS := -lpthread -ldl

# ── source lists ────────────────────────────────────────
CIVET_SRC := third_party/civetweb/civetweb.c third_party/civetweb/civetweb.h
COMMON_SRC  := \
    src/config.c \
    src/key_loader.c \
    src/ws_proto.c \
